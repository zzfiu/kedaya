...
class Main extends Template {
    ...
    async prepare() {
        ...
        for (let cookie of this.cookies.help) {
            let s = await this.algo.curl({
                    'url': `https://api.m.jd.com/?functionId=runningTeamInfo&body={"linkId":"L-sOanK_5RJCz7I314FpnQ"}&t=1660920601386&appid=activities_platform&client=ios&clientVersion=4.5.0&cthr=1&build=1164&screen=390*844&networkType=wifi&d_brand=iPhone&d_model=iPhone13,3&lang=zh_CN&osVersion=15.1.1&partner=`,
                    cookie
                }
            )
    ...
    async main(p) {
        ...
        let home = await this.algo.curl({
                'url': `https://api.m.jd.com/?functionId=runningPageHome&body={"linkId":"L-sOanK_5RJCz7I314FpnQ","isFromJoyPark":true,"joyLinkId":"LsQNxL7iWDlXUs6cFl-AAg"}&t=1660980238110&appid=activities_platform&client=ios&clientVersion=4.5.0&cthr=1&build=1164&screen=320*568&networkType=wifi&d_brand=iPhone&d_model=iPhone8,4&lang=zh_CN&osVersion=11.4&partner=`,
                cookie
            }
        )
        let risk = 0
            ...
            for (let k of this.code) {
                if (!k.finish) {
                    ...
                    let join = await this.algo.curl({
                            'url': `https://api.m.jd.com/?functionId=runningJoinTeam&body={"linkId":"L-sOanK_5RJCz7I314FpnQ","captainId":"${k.captainId}"}&t=1660920651446&appid=activities_platform&client=ios&clientVersion=4.5.0`,
                            // 'form':``,
                            cookie,
                            algo: {
                                appId: "448de",
        ...
        if (!tt && this.code.length>0) {
            console.log("🦆  测试风控...")
            let join = await this.algo.curl({
                    'url': `https://api.m.jd.com/?functionId=runningJoinTeam&body={"linkId":"L-sOanK_5RJCz7I314FpnQ","captainId":"${this.random(this.code, 1)[0].captainId}"}&t=1660920651446&appid=activities_platform&client=ios&clientVersion=4.5.0`,
                    // 'form':``,
                    cookie,
                    algo: {
                        appId: "448de",
        ...
        let task = await this.algo.curl({
                'url': `https://api.m.jd.com/?functionId=apTaskList&body={"linkId":"L-sOanK_5RJCz7I314FpnQ"}&t=1660910738295&appid=activities_platform&client=ios&clientVersion=4.5.0&cthr=1&build=1164&screen=390*844&networkType=wifi&d_brand=iPhone&d_model=iPhone13,3&lang=zh_CN&osVersion=15.1.1&partner=`,
                // 'form':``,
                cookie
            }
        )
        ...
        for (let z of Array(8)) {
                ...
                for (let nn in this.profile) {
                    if (this.match(/assets_\d+_\d+/, nn)) {
                        ...
                        if (parseInt(n)<=hours && parseInt(m)>hours) {
                            ...
                            console.log(`🐻  选择时间段: ${n}-${m} , 目标: ${assets}`)
                        }
                    }
                }
            }
            if (info.nextRunningTime && !info.energy) {
                ...
                console.log(`🐢  距离下次跑路还有${next}秒, 如果小于${wait}秒,将执行等待`)
                if (next<parseInt(wait)) {
                    console.log(`🐴  等待: ${next} 秒`)
                    await this.wait(next * 1000)
                    home = await this.algo.curl({
                            'url': `https://api.m.jd.com/?functionId=runningPageHome&body={"linkId":"L-sOanK_5RJCz7I314FpnQ"}&t=1660980238110&appid=activities_platform&client=ios&clientVersion=4.5.0&cthr=1&build=1164&screen=320*568&networkType=wifi&d_brand=iPhone&d_model=iPhone8,4&lang=zh_CN&osVersion=11.4&partner=`,
                            cookie
                        }
                    )
            ...
            if (info.nextRunningTime && info.energy) {
                ...
                let bar = await this.algo.curl({
                        ...
                        'form': `functionId=runningUseEnergyBar&body={"linkId":"L-sOanK_5RJCz7I314FpnQ"}&t=1660919679853&appid=activities_platform&client=ios&clientVersion=4.5.0`,
                        cookie
                    }
                )
            ...
            if (!info.nextRunningTime) {
                ...
                for (let i = 0; i<10; i++) {
                    let box = await this.algo.curl({
                            'url': `https://api.m.jd.com/`,
                            'form': `functionId=runningOpenBox&body={"linkId":"L-sOanK_5RJCz7I314FpnQ"}&t=1660919679853&appid=activities_platform&client=ios&clientVersion=4.5.0`,
                            cookie,
                            algo: {
                                appId: "b6ac3",
                                version: "3.1",
                    ...
                    if (parseFloat(data.assets)>=assets) {
                        ...
                        let as = await this.algo.curl({
                                'url': `https://api.m.jd.com/`,
                                'form': `functionId=runningPreserveAssets&body={"linkId":"L-sOanK_5RJCz7I314FpnQ"}&t=1660920651446&appid=activities_platform&client=ios&clientVersion=4.5.0`,
                                cookie
                            }
                        )
        ...
        let team = await this.algo.curl({
                'url': `https://api.m.jd.com/?functionId=runningTeamInfo&body={"linkId":"L-sOanK_5RJCz7I314FpnQ"}&t=1660920651446&appid=activities_platform&client=ios&clientVersion=4.5.0`,
                // 'form':``,
                cookie,
                algo: {
                    appId: "448de",
        ...
        if (this.haskey(team, 'data.teamSumPrize')) {
            this.print(`🦉  战队总奖金: ${team.data.teamSumPrize}`, p.user)
        }
